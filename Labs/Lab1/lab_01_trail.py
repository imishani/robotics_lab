...
def create_angular_action(actuator_count, base_cyclic):
    ...
    for joint_id in range(actuator_count):
        ...
        joint_angle.value = base_cyclic.RefreshFeedback().actuators[joint_id].position + 1
...
def create_cartesian_action(base_cyclic):
    ...
    while not ok:
        ...
        d_x = float(input("Enter required delta y for the end effector position"))
...
def example_create_sequence(base, base_cyclic):
    ...
    angular_action = create_angular_action(actuator_count,base_cyclic)
...
if __name__ == "__main__":
    ...
    try:
        ...
        with utilities.DeviceConnection.createTcpConnection(args) as router:
            ...
            # Example core
            success = True
            input("Press Enter to move the arm  to home position")
            ...
            input("Press Enter to move the arm to desired position")
            success &= example_create_sequence(base, base_cyclic)
            if success:
                print('Successfully moved to arm to desired position')
            else:
                print('Huston, we have a problem, please call the instructor')
