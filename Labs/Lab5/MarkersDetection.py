...
class cameraScreen:
    def __init__(self):
        ...
        edge_centers = np.divide(np.add(self.corners[0][1, :], self.corners[0][0, :]), 2)
        centers = []
    ...
    def updateGraph(self):
        ...
        x, y = [], []
        for i in range(0, len(self.arucosList)):
            newCenters.append({"id": self.arucosList[i]["id"], "cX": self.arucosList[i]["cX"] - cX_0,
                               "cY": self.arucosList[i]["cY"] - cY_0,
                               "distance": ((self.arucosList[i]["cX"] - cX_0) ** 2 +
                                            (self.arucosList[i]["cY"] - cY_0) ** 2) ** 0.5})
